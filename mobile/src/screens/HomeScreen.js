...
import * as Battery from 'expo-battery'; import AsyncStorage from '@react-native-async-storage/async-storage';
import api from '../services/api';

export default function HomeScreen({ navigation }) {
  useEffect(() => {
    const sync = async (status = 'Online') => {
      try {
        const user = JSON.parse(await AsyncStorage.getItem('user'));
        if (!user) return;
        const level = await Battery.getBatteryLevelAsync();
        await api.put('/telemetry/sync', { deviceId: user.deviceId, battery: status === 'Offline' ? 'Unknown' : Math.round(level * 100) + '%', status });
      } catch (err) { console.log(err); }
    };
    sync(); const interval = setInterval(() => sync('Online'), 60000);
    return () => clearInterval(interval);
  }, []);

  const handleLogout = async () => {
    const user = JSON.parse(await AsyncStorage.getItem('user'));
    if (user) await api.put('/telemetry/sync', { deviceId: user.deviceId, status: 'Offline' });
...
const styles = StyleSheet.create({
  ...
  container: { flex: 1, paddingHorizontal: 25 }, welcomeSection: { marginTop: 30, marginBottom: 40 },
  greeting: { fontSize: 32, fontWeight: '800', color: '#f8fafc', marginBottom: 8 }, subGreeting: { fontSize: 16, color: '#94a3b8', lineHeight: 24 },
  sectionTitle: { fontSize: 18, fontWeight: '700', color: '#f8fafc', marginBottom: 15, letterSpacing: 0.5 }, sectionTitleTask: { fontSize: 18, fontWeight: '700', color: '#f8fafc', marginTop: 15, marginBottom: 15, letterSpacing: 0.5 },
  ...
  cardTitle: { fontSize: 18, fontWeight: '700', color: '#f8fafc' }, badge: { backgroundColor: 'rgba(56, 189, 248, 0.1)', paddingVertical: 4, paddingHorizontal: 10, borderRadius: 12 },
  badgeText: { color: '#38bdf8', fontSize: 12, fontWeight: 'bold' }, cardDescription: { color: '#94a3b8', fontSize: 14, lineHeight: 22 },
  ...
  taskTitle: { fontSize: 18, fontWeight: '800', color: '#0f172a' }, taskBadge: { backgroundColor: '#0f172a', paddingVertical: 4, paddingHorizontal: 10, borderRadius: 12 },
  taskBadgeText: { color: '#38bdf8', fontSize: 12, fontWeight: 'bold' }, taskDescription: { color: '#0f172a', fontSize: 14, fontWeight: '600', lineHeight: 22 }
});